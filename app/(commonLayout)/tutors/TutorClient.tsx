...
import { GraduationCap, SlidersHorizontal, X, ChevronLeft, ChevronRight } from "lucide-react";
...
interface TutorsClientProps {
  ...
  initialMinRate: string | number;
  initialMaxRate: string | number;
  initialExperience: string | number;
}
...
export default function TutorsClient({
  ...
  const [localMin, setLocalMin] = useState(String(initialMinRate || ""));
  const [localMax, setLocalMax] = useState(String(initialMaxRate || ""));
  const [localExp, setLocalExp] = useState(String(initialExperience || ""));

  const currentPage = Number(searchParams.get("page") || initialPage);
  const sortBy = searchParams.get("sortBy") || initialSortBy;
  const sortOrder = searchParams.get("sortOrder") || initialSortOrder;
            ...
            <div>
              ...
              {!loading && (
                <p className="text-muted-foreground mt-2 text-sm">
                  {totalCount > 0
                    ? <>
                        {totalCount} tutor{totalCount !== 1 ? "s" : ""} available · Page {currentPage} of {totalPages || 1}
                      </>
        ...
        {!loading && tutors.length > 0 && (
          <div className="flex items-center justify-center gap-3 mt-14">
            ...
            <div className="flex items-center gap-1">
              ...
              {(totalPages || 1) > 5 && <span className="text-muted-foreground px-1">...</span>}
            </div>
